...
const Attendent = {
  ...
  create: async (data) => {
    const { student_id, class_id, date, status, remarks } = data;
  ...
  createBulk: async (data) => {
    const sql = 'INSERT INTO attendents (student_id, class_id, date, status, remarks) VALUES ?';
    const values = data.map(item => [item.student_id, item.class_id, item.date, item.status, item.remarks]);
  ...
  getClassAttendanceByDate: async (classId, date) => {
    const sql = 'SELECT a.*, s.name as student_name FROM attendents a JOIN students s ON a.student_id = s.id WHERE a.class_id = ? AND a.date = ?';
  ...
  getClassReport: async (classId, month, year) => {
    const sql = `
        ...
        FROM 
            students s
        LEFT JOIN 
            attendents a ON s.id = a.student_id AND a.class_id = ? AND MONTH(a.date) = ? AND YEAR(a.date) = ?
        WHERE 
            s.class_id = ?
        GROUP BY 
            s.id, s.name;
